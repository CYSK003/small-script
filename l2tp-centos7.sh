...
# 获取服务器IP
serverip=$(ifconfig -a |grep -w "inet"| grep -v "127.0.0.1" |awk '{print $2;}' | head -n 1)
serveriplist=$(ifconfig -a |grep -w "inet"| grep -v "127.0.0.1" |awk '{print $2;}')
if [[ $(printf "$serveriplist\n" | wc -l) -gt 1 ]]; then
    #statements
    echo ======================================
    echo "IP list:"
    printf "\e[33m$serveriplist\e[0m\n"
    echo ======================================
fi
printf "\e[33m$serverip\e[0m is the internet server IP?\n"
...
printf "(Default PSK: \e[33m$mypsk\e[0m): "
read mypsktmp
if [[ -n "$mypsktmp" ]]; then
    mypsk=$mypsktmp
fi
...
printf "(Default VPN username: \e[33mvpn\e[0m): "
read usernametmp
...
# 安装依赖组件
yum -y install openswan ppp xl2tpd wget iptables iptables-services
...
# 允许开机启动
systemctl enable ipsec xl2tpd iptables
systemctl restart ipsec xl2tpd iptables

clear

# 这里暂停5s让ipsec/xl2tpd/iptables启动完
echo "waiting 5s ...."
sleep 5s
